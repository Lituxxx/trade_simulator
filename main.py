...
# 绘制动态K线图
def plot_dynamic_candlestick(df, start_idx, end_idx, highlight_idx=None, selected_stock=None):
    """使用Plotly绘制动态K线图（修复非交易日中断问题，兼容旧版本Plotly）"""
    # 切片数据
    plot_data = df.iloc[start_idx:end_idx].copy()
    
    # 为交易日创建连续索引（从1开始）
    plot_data['trading_day_index'] = range(1, len(plot_data) + 1)
    
    # 计算每天涨跌情况
    plot_data['is_increasing'] = plot_data['close'] >= plot_data['open']
    
    # 生成K线图悬停文本
    plot_data['hover_text'] = (
        '日期: ' + plot_data['trade_date'].dt.strftime('%Y-%m-%d') + '<br>' +
        '开盘: ' + plot_data['open'].apply(lambda x: f'{x:.2f}') + '<br>' +
        '最高: ' + plot_data['high'].apply(lambda x: f'{x:.2f}') + '<br>' +
        '最低: ' + plot_data['low'].apply(lambda x: f'{x:.2f}') + '<br>' +
        '收盘: ' + plot_data['close'].apply(lambda x: f'{x:.2f}')
    )
    ...
    fig.add_trace(go.Candlestick(
        ...
        name=selected_stock,
        hovertext=plot_data['hover_text']  # 使用hovertext替代hovertemplate
    ))
    
    # 分离上涨和下跌的成交量数据
    increasing_vol = plot_data[plot_data['is_increasing']][['trading_day_index', 'vol', 'trade_date']]
    decreasing_vol = plot_data[~plot_data['is_increasing']][['trading_day_index', 'vol', 'trade_date']]
    
    # 设置子图布局，K线图占8/9高度，成交量图占1/9高度
    fig.update_layout(
        title="股票K线图（连续交易日）",
        xaxis=dict(
            domain=[0, 1],
        ...
        yaxis=dict(
            domain=[1/9, 1],  # K线图占据上方8/9的高度
            title="价格 (元)"
        ),
        xaxis2=dict(
            domain=[0, 1],
            anchor='y2',
            tickvals=plot_data['trading_day_index'],
            ticktext=plot_data['trade_date'].dt.strftime('%Y-%m-%d'),
            tickangle=0
        ),
        yaxis2=dict(
            domain=[0, 1/5],  # 成交量图占据下方1/5的高度
            title="成交量"
        ),
        hovermode="x unified",
    ...
    # 添加上涨成交量柱状图（绿色）并设置悬停文本
    if not increasing_vol.empty:
        increasing_vol['vol_hover_text'] = (
            '日期: ' + increasing_vol['trade_date'].dt.strftime('%Y-%m-%d') + '<br>' +
            '成交量: ' + increasing_vol['vol'].apply(lambda x: f'{x}')
        )
        fig.add_trace(go.Bar(
            x=increasing_vol['trading_day_index'],
            y=increasing_vol['vol'] if 'vol' in increasing_vol.columns else np.zeros(len(increasing_vol)),
            name='上涨成交量',
            marker_color='green',
            xaxis='x2',
            yaxis='y2',
            hovertext=increasing_vol['vol_hover_text']  # 使用hovertext替代hovertemplate
        ))
    
    # 添加下跌成交量柱状图（红色）并设置悬停文本
    if not decreasing_vol.empty:
        decreasing_vol['vol_hover_text'] = (
            '日期: ' + decreasing_vol['trade_date'].dt.strftime('%Y-%m-%d') + '<br>' +
            '成交量: ' + decreasing_vol['vol'].apply(lambda x: f'{x}')
        )
        fig.add_trace(go.Bar(
            x=decreasing_vol['trading_day_index'],
            y=decreasing_vol['vol'] if 'vol' in decreasing_vol.columns else np.zeros(len(decreasing_vol)),
            name='下跌成交量',
            marker_color='red',
            xaxis='x2',
            yaxis='y2',
            hovertext=decreasing_vol['vol_hover_text']  # 使用hovertext替代hovertemplate
        ))
...
def main():
    ...
    if 'trading_days' not in st.session_state:
        st.session_state.trading_days = 100  # 初始显示100个交易日，动态加载更多
    if 'buy_executed' not in st.session_state:
        st.session_state.buy_executed = False  # 标记是否执行了买入操作
    if 'data_loaded_range' not in st.session_state:
        st.session_state.data_loaded_range = (0, 100)  # 已加载数据的范围
    if 'max_data_length' not in st.session_state:
        st.session_state.max_data_length = 0  # 数据总长度
    ...
    with st.sidebar:
        ...
        if mode != st.session_state.mode_selected:
            ...
            st.session_state.selected_stock_loaded = None
            st.session_state.auto_loaded = False
            st.session_state.data_loaded_range = (0, 100)
            ...
            if not available_stocks:
                ...
                if uploaded_file is not None:
                    ...
                    try:
                        ...
                        if 'trade_date' in df.columns:
                            ...
                            st.session_state.max_data_length = len(df)
                            st.success("数据加载成功！")
                        else:
                ...
                if st.button("加载选中的股票"):
                    ...
                    if not stock_data.empty:
                        ...
                        st.session_state.max_data_length = len(stock_data)
    ...
    if hasattr(st.session_state, 'stock_data') and not st.session_state.stock_data.empty and st.session_state.stock_data_loaded:
        ...
        # 确保max_data_length已设置
        if st.session_state.max_data_length == 0:
            st.session_state.max_data_length = total_days
        
        # 解包已加载数据范围
        start_loaded, end_loaded = st.session_state.data_loaded_range
        
        # 计算当前显示范围（始终显示已加载数据的全部）
        current_start = start_loaded
        current_end = end_loaded
        
        # 获取当前日期索引（已加载数据的最后一个）
        current_day = end_loaded - 1
        ...
        # 绘制动态K线图
        st.subheader(f"股票K线图 (第{current_start+1}至第{current_end}个交易日)")
        if current_end > current_start:
            # 计算当前日期在已加载数据中的索引
            highlight_idx = current_day - current_start if current_day >= current_start else None
            fig = plot_dynamic_candlestick(
                stock_data, 
                current_start, 
                current_end, 
                highlight_idx,
        ...
        with col1:
            if st.button("加载更多数据") and end_loaded < total_days:
                # 每次加载50个交易日
                new_end = min(end_loaded + 50, total_days)
                st.session_state.data_loaded_range = (start_loaded, new_end)
                st.rerun()
            elif end_loaded >= total_days:
                st.info("已加载全部数据")

        with col2:
            if st.button("前进1个交易日"):
                # 计算下一个交易日索引
                next_day = current_day + 1
                if next_day < total_days:
                    # 确保数据已加载到下一个交易日
                    if next_day >= end_loaded:
                        st.session_state.data_loaded_range = (start_loaded, min(end_loaded + 50, total_days))
        ...
        with col3:
            if st.button("前进7个交易日"):
                ...
                # 确保数据已加载到目标日期
                if target_day >= end_loaded:
                    st.session_state.data_loaded_range = (start_loaded, min(end_loaded + max(50, target_day - end_loaded + 10), total_days))
        
                st.session_state.current_day = target_day
                st.rerun()

        with col4:
            if st.button("前进30个交易日"):
                # 计算30个交易日后的索引
                target_day = current_day + 30
                if target_day >= total_days:
                    target_day = total_days - 1
